...
/*	public function exhibit_Notice() {
		$sql = "SELECT buyer_id,e.product_name,b.member_id FROM t_exhibit e INNER JOIN t_exhibit_image i ON e.exhibit_id = i.exhibit_id INNER JOIN t_buyer b ON b.exhibit_id= e.exhibit_id Where e.flg = 1 AND i.line=1 ORDER by e.day";
		$stmt = $this->db->prepare($sql);
		$result = $stmt->execute();
		$small = null;
		$exhibit_List = array();
		...
		$exhibit = new Exhibit();
		$exhibit->setBuy_id($buyer_id);
		$exhibit->setProduct_name($product_name);
		$exhibit->setMember($member_id);
		$exhibit_List[$buyer_id] = $exhibit;
	}
		return $exhibit_List;
	}*/
	public function exhibit_Notice() {
		$sql = "SELECT b.exhibit_id,e.product_name,b.member_id FROM t_exhibit e INNER JOIN t_exhibit_image i ON e.exhibit_id = i.exhibit_id INNER JOIN t_buyer b ON b.exhibit_id= e.exhibit_id Where e.flg = 1 AND i.line=1 ORDER by e.day";
		$stmt = $this->db->prepare($sql);
		$result = $stmt->execute();
		$small = null;
		$exhibit_List = array();
		...
		$exhibit->setProduct_name($product_name);
		$exhibit->setMember($member_id);
		$exhibit_List[$exhibit_id] = $exhibit;
	}
		return $exhibit_List;
	}
	...
	public function exhibitIdcount($id) {
		$sql = "SELECT count(*) FROM t_exhibit e INNER JOIN t_exhibit_image i ON e.exhibit_id = i.exhibit_id INNER JOIN m_smallcategory m ON e.smallcategory_id = m.smallcategory_id Where e.flg = 0 AND i.line=1 AND e.smallcategory_id =:smallcategory_id ORDER by e.day ";
		$stmt = $this->db->prepare($sql);
	...
	public function exhibitId_keycount($key,$order,$id) {
		$sql = "SELECT count(*) FROM t_exhibit e INNER JOIN t_exhibit_image i ON e.exhibit_id = i.exhibit_id INNER JOIN m_smallcategory m ON e.smallcategory_id = m.smallcategory_id Where i.line=1 AND e.smallcategory_id =:smallcategory_id AND e.flg = 0  ORDER by e.$key $order";
		$stmt = $this->db->prepare($sql);
		$stmt->bindValue(":smallcategory_id",$id, PDO::PARAM_INT);
		$result = $stmt->execute();
		$small = null;
	...
	public function exhibitId_keywordcount($keywords,$key,$order) {
		$sql = "SELECT count(*) FROM t_exhibit e INNER JOIN t_exhibit_image i ON e.exhibit_id = i.exhibit_id INNER JOIN m_smallcategory s ON e.smallcategory_id = s.smallcategory_id INNER JOIN m_mediumcategory m ON s.mediumcategory_id = m.mediumcategory_id INNER JOIN m_bigcategory big ON m.bigcategory_id = big.bigcategory_id 
		    Where i.line=1 AND e.remarks LIKE :remarks AND e.flg = 0 or
		          ...
		          i.line=1 AND big.category_name LIKE :big_keywords AND e.flg = 0 ORDER by e.$key $order";
		$stmt = $this->db->prepare($sql);
		$stmt->bindValue(":remarks","%".$keywords."%",PDO::PARAM_STR);
		$stmt->bindValue(":product_name","%".$keywords."%",PDO::PARAM_STR);
		$stmt->bindValue(":s_keywords","%".$keywords."%",PDO::PARAM_STR);
	...
	public function exhibit_keywordcount($keywords) {
		$sql = "SELECT count(*) FROM t_exhibit e INNER JOIN t_exhibit_image i ON e.exhibit_id = i.exhibit_id INNER JOIN m_smallcategory s ON e.smallcategory_id = s.smallcategory_id INNER JOIN m_mediumcategory m ON s.mediumcategory_id = m.mediumcategory_id INNER JOIN m_bigcategory big ON m.bigcategory_id = big.bigcategory_id 
		    Where i.line=1 AND e.remarks LIKE :remarks AND e.flg = 0 or
		          i.line=1 AND product_name LIKE :product_name AND e.flg = 0 or 
		          i.line=1 AND s.category_name LIKE :s_keywords AND e.flg = 0 or 
		          i.line=1 AND m.category_name LIKE :m_keywords AND e.flg = 0 or 
		          i.line=1 AND big.category_name LIKE :big_keywords AND e.flg = 0";
		$stmt = $this->db->prepare($sql);
		$stmt->bindValue(":remarks","%".$keywords."%",PDO::PARAM_STR);
		$stmt->bindValue(":product_name","%".$keywords."%",PDO::PARAM_STR);
		$stmt->bindValue(":s_keywords","%".$keywords."%",PDO::PARAM_STR);
	...
	public function Id_count($id) {
		$sql = "SELECT count(*) FROM t_exhibit e Where e.member_id =:member_id ";
		$stmt = $this->db->prepare($sql);
	...
	public function t_coment($exhibit_id){
			$sql = "SELECT sei,mei,num,c.time,e.exhibit_id,coment_name FROM t_exhibit e INNER JOIN t_coment c ON e.exhibit_id = c.exhibit_id INNER JOIN m_member m ON c.member_id = m.member_id where e.exhibit_id =:exhibit_id";
			$stmt = $this->db->prepare($sql);
			$stmt->bindValue(":exhibit_id",$exhibit_id,PDO::PARAM_INT);
			$result = $stmt->execute();
			$comentList = array();
